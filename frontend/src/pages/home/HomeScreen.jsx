    ...
    <>
      <div className="relative h-screen text-white"> 
        ...
        <img src={ORIGINAL_IMG_BASE_URL + trendingContent?.backdrop_path} alt="hero img" 
          className="absolute top-0 left-0 size-full object-cover -z-50" onLoad={() =>  setImgLoading(false)}
        />

        <div className="absolute top-0 left-0 size-full bg-black/50 -z-50" aria-hidden="true"/>

        <div className="absolute top-0 left-0 size-full flex flex-col justify-center px-8 md:px-16 lg:px-32">
          <div className="bg-gradient-to-b from-black via-transparent to-transparent absolute size-full top-0 left-0 -z-10"/>
          <div className="max-w-2xl">
            <h1 className="mt-4 text-6xl font-extrabold text-balance">{trendingContent?.title || trendingContent?.name}</h1>
            <p className="mt-2 text-lg">{new Date(trendingContent?.release_date || trendingContent?.first_air_date).getFullYear()}{trendingContent?.adult ? " | 18+" :""}</p>
